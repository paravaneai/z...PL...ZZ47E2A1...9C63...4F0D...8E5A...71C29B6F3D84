(function () {
    "use strict";

    var storageKey = "paravane-theme";
    var siteConfig = window.ParavaneSiteConfig || {};
    var themeConfig = siteConfig.theme || {};
    var switchingEnabled = themeConfig.switchingEnabled === true;
    var defaultTheme = themeConfig.defaultTheme === "light" ? "light" : "dark";
    var root = document.documentElement;
    var localHostnames = ["localhost", "127.0.0.1", "::1"];
    var API_BASE = window.PARAVANE_API_BASE ||
        (localHostnames.indexOf(window.location.hostname) >= 0 ? "" : "https://api.paravane.io");
    var themeChangeReleaseFrame = 0;
    var faviconPaths = {
        dark: "../../../shared/b4/b47d8a12345bdbf064f0de8351c8ff55b0c87d6eec4e283d71a72b3fa8e150eb.ico",
        light: "../../../shared/47/47f41a0063dc5d27def2b6ab47b53a7d22ee2aefecb2a84765c9e6c1539af98c.ico"
    };

    root.setAttribute(
        "data-theme-switching",
        switchingEnabled ? "enabled" : "disabled"
    );

    function isValidTheme(theme) {
        return theme === "dark" || theme === "light";
    }

    function storedTheme() {
        try {
            var theme = window.localStorage.getItem(storageKey);
            return isValidTheme(theme) ? theme : defaultTheme;
        } catch (err) {
            return defaultTheme;
        }
    }

    function persistTheme(theme) {
        if (!switchingEnabled) return;

        try {
            window.localStorage.setItem(storageKey, theme);
        } catch (err) {}
    }

    function currentTheme() {
        var theme = root.getAttribute("data-theme");
        return isValidTheme(theme) ? theme : defaultTheme;
    }

    function beginAtomicThemeChange() {
        if (themeChangeReleaseFrame) {
            window.cancelAnimationFrame(themeChangeReleaseFrame);
            themeChangeReleaseFrame = 0;
        }

        root.classList.add("theme-changing");

        /*
         * Commit transition suppression before data-theme changes. Without this
         * read, Chromium may batch the class and theme attribute into one style
         * recalculation and allow existing theme-sensitive transitions to start.
         */
        void root.offsetWidth;
    }

    function endAtomicThemeChange() {
        themeChangeReleaseFrame = window.requestAnimationFrame(function () {
            themeChangeReleaseFrame = window.requestAnimationFrame(function () {
                root.classList.remove("theme-changing");
                themeChangeReleaseFrame = 0;
            });
        });
    }

    function updateToggle(theme) {
        var nextTheme = theme === "dark" ? "light" : "dark";
        var label = theme === "dark" ? "Dark" : "Light";
        var title = "Switch to " + nextTheme + " theme";

        document.querySelectorAll("[data-theme-toggle]").forEach(function (button) {
            button.hidden = !switchingEnabled;
            button.disabled = !switchingEnabled;
            button.setAttribute("aria-hidden", switchingEnabled ? "false" : "true");

            if (!switchingEnabled) return;

            button.setAttribute("aria-label", title);
            button.setAttribute("aria-pressed", theme === "light" ? "true" : "false");
            button.setAttribute("aria-checked", theme === "dark" ? "true" : "false");
            button.setAttribute("title", title);

            var labelEl = button.querySelector("[data-theme-toggle-label]");
            if (labelEl) {
                labelEl.textContent = label;
            }
        });
    }

    function syncLoaderTheme(theme) {
        var overlay = document.querySelector(".animationload");
        var loader = document.querySelector(".loader");
        var isLight = theme === "light";
        var imageRegistry = window.ParavaneImages;
        var loaderImage = imageRegistry
            ? imageRegistry.get(isLight ? "brand.loader.light" : "brand.loader.dark")
            : "";

        if (overlay) {
            overlay.style.background = isLight ? "#ffffff" : "#000000";
        }

        if (loader) {
            loader.style.backgroundImage = loaderImage ? "url('" + loaderImage + "')" : "";
        }
    }

    function syncThemeMedia(theme) {
        var srcAttr = theme === "light" ? "data-theme-src-light" : "data-theme-src-dark";
        var srcsetAttr = theme === "light" ? "data-theme-srcset-light" : "data-theme-srcset-dark";

        document.querySelectorAll("[data-theme-src-dark][data-theme-src-light]").forEach(function (image) {
            var nextSrc = image.getAttribute(srcAttr);
            if (nextSrc && image.getAttribute("src") !== nextSrc) {
                image.setAttribute("src", nextSrc);
            }
        });

        document.querySelectorAll("[data-theme-srcset-dark][data-theme-srcset-light]").forEach(function (source) {
            var nextSrcset = source.getAttribute(srcsetAttr);
            if (nextSrcset && source.getAttribute("srcset") !== nextSrcset) {
                source.setAttribute("srcset", nextSrcset);
            }
        });
    }

    function syncFavicon(theme) {
        var favicon = document.querySelector('link[rel~="icon"]');
        var nextHref = faviconPaths[theme === "light" ? "light" : "dark"];

        if (!favicon) {
            favicon = document.createElement("link");
            favicon.setAttribute("rel", "icon");
            document.head.appendChild(favicon);
        }

        if (favicon.getAttribute("href") !== nextHref) {
            favicon.setAttribute("href", nextHref);
        }
    }

    function syncThemeUi(theme) {
        syncLoaderTheme(theme);
        syncThemeMedia(theme);
        syncFavicon(theme);
        updateToggle(theme);
    }

    function setTheme(theme, options) {
        var previousTheme = currentTheme();

        if (!switchingEnabled) theme = defaultTheme;
        if (!isValidTheme(theme)) theme = defaultTheme;

        var themeChanged = previousTheme !== theme || !root.hasAttribute("data-theme");

        /*
         * The stylesheet owns color-scheme. Keeping it out of an inline style
         * prevents Chromium or a color-conversion extension from interpreting
         * a repeated inline assignment during navigation as a new rendering
         * mode. The document also opts out of generated dark themes in <head>.
         */
        root.style.removeProperty("color-scheme");

        if (themeChanged) {
            beginAtomicThemeChange();
        }

        try {
            if (themeChanged) {
                root.setAttribute("data-theme", theme);
            }

            if (!options || options.persist !== false) {
                persistTheme(theme);
            }

            syncThemeUi(theme);
        } finally {
            if (themeChanged) {
                endAtomicThemeChange();
            }
        }

        if ((!options || !options.silent) && previousTheme !== theme) {
            window.dispatchEvent(new CustomEvent("paravane:themechange", {
                detail: { theme: theme }
            }));
        }
    }

    function toggleTheme() {
        if (!switchingEnabled) return;
        setTheme(currentTheme() === "dark" ? "light" : "dark");
    }

    function restoreTheme() {
        var expectedTheme = switchingEnabled ? storedTheme() : defaultTheme;

        if (currentTheme() !== expectedTheme || root.style.getPropertyValue("color-scheme")) {
            setTheme(expectedTheme, {
                silent: true,
                persist: false
            });
            return;
        }

        syncThemeUi(expectedTheme);
    }

    restoreTheme();

    function authenticatedAccountMarkup() {
        return '<span class="nav-session-action__icon" aria-hidden="true">' +
            '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round">' +
                '<circle cx="12" cy="8" r="3.25"></circle>' +
                '<path d="M5.8 19.1c.8-3.2 3-5 6.2-5s5.4 1.8 6.2 5"></path>' +
            '</svg>' +
        '</span><span>Account</span>';
    }

    function sessionNavigationActions() {
        return Array.prototype.slice.call(document.querySelectorAll(
            'a.nav-action[href^="../pages/auth/signin"], a.nav-session-action'
        ));
    }

    function showAuthenticatedNavigation(actions) {
        actions.forEach(function (action) {
            action.classList.add("nav-session-action");
            action.classList.remove("active");
            action.setAttribute("data-session-state", "authenticated");
            action.setAttribute("href", "../pages/app/");
            action.setAttribute("aria-label", "Open account dashboard");
            action.setAttribute("title", "Open account dashboard");
            action.innerHTML = authenticatedAccountMarkup();
        });
        root.classList.add("site-session-authenticated");
    }

    function notifySessionState(state) {
        state = state === "authenticated" || state === "guest" ? state : "unknown";
        root.classList.toggle("site-session-authenticated", state === "authenticated");
        root.classList.toggle("site-session-guest", state === "guest");
        root.classList.toggle("site-session-checked", state !== "unknown");
        root.setAttribute("data-site-session-state", state);
        window.dispatchEvent(new CustomEvent("paravane:sessionstatechange", {
            detail: {
                state: state,
                authenticated: state === "authenticated"
            }
        }));
    }

    async function refreshSessionNavigation() {
        var actions = sessionNavigationActions();

        if (!actions.length || document.querySelector('form[data-auth-form="login"]')) {
            notifySessionState("unknown");
            return;
        }

        try {
            var response = await fetch(API_BASE + "../v1/auth/me", {
                credentials: "include",
                headers: { "Accept": "application/json" }
            });
            if (response.status === 401 || response.status === 403) {
                notifySessionState("guest");
                return;
            }

            if (!response.ok) {
                notifySessionState("unknown");
                return;
            }

            var result = await response.json().catch(function () { return {}; });
            var tenant = result && result.tenant;
            var authenticated = Boolean(result && (
                result.authenticated === true ||
                result.user ||
                tenant
            ));

            if (result && result.authenticated === false) {
                notifySessionState("guest");
                return;
            }

            if (!authenticated) {
                notifySessionState("unknown");
                return;
            }

            if (tenant && String(tenant.status || "").toLowerCase() === "active") {
                showAuthenticatedNavigation(actions);
            }

            notifySessionState("authenticated");
        } catch (err) {
            notifySessionState("unknown");
        }
    }

    if (document.readyState === "loading") {
        document.addEventListener("DOMContentLoaded", refreshSessionNavigation, { once: true });
    } else {
        refreshSessionNavigation();
    }

    function bindThemeControls() {
        restoreTheme();

        document.querySelectorAll("[data-theme-toggle]").forEach(function (button) {
            if (!switchingEnabled || button.getAttribute("data-theme-toggle-bound") === "true") {
                return;
            }

            button.setAttribute("data-theme-toggle-bound", "true");
            button.addEventListener("click", toggleTheme);
        });
    }

    function bindMobileNavigation() {
        var header = document.querySelector(".site-header");

        if (!header || header.getAttribute("data-mobile-nav-bound") === "true") {
            return;
        }

        var nav = header.querySelector(".nav");
        var links = header.querySelector(".nav-links");
        var actions = header.querySelector(".nav-actions");

        if (!nav || !links || !actions) {
            return;
        }

        header.setAttribute("data-mobile-nav-bound", "true");

        var mobileQuery = window.matchMedia("(max-width: 720px)");
        var catalogs = Array.prototype.slice.call(links.querySelectorAll(".nav-catalog"));
        var menuOpenScrollY = 0;
        var scrollDismissTravel = 4;

        if (!links.id) {
            links.id = "primary-navigation";
        }

        var button = document.createElement("button");
        button.className = "mobile-nav-toggle";
        button.type = "button";
        button.setAttribute("aria-label", "Open navigation menu");
        button.setAttribute("aria-controls", links.id);
        button.setAttribute("aria-expanded", "false");
        button.innerHTML = '<span class="mobile-nav-toggle__icon" aria-hidden="true"></span>';
        actions.appendChild(button);

        function menuIsOpen() {
            return button.getAttribute("aria-expanded") === "true";
        }

        function documentScrollY() {
            var scroller = document.scrollingElement || root;
            var maxScrollY = Math.max(0, scroller.scrollHeight - scroller.clientHeight);
            return Math.min(Math.max(window.scrollY || 0, 0), maxScrollY);
        }

        function usesCatalogDisclosure(event) {
            var pointerType = event && typeof event.pointerType === "string" ? event.pointerType : "";

            return mobileQuery.matches || pointerType === "touch" || pointerType === "pen" ||
                window.matchMedia("(hover: none), (pointer: coarse)").matches;
        }

        function updateMenuGeometry() {
            var viewportHeight = window.visualViewport ? window.visualViewport.height : window.innerHeight;
            var headerHeight = Math.ceil(header.getBoundingClientRect().height);

            /* The panel belongs to the header, not to a fixed-position viewport. */
            header.style.setProperty("--mobile-nav-available-height", Math.max(0, viewportHeight - headerHeight) + "px");
        }

        function revealHeader() {
            header.classList.remove("is-scroll-collapsed");
            header.classList.add("is-scroll-expanded");
            root.classList.remove("site-header-is-collapsed");
            root.style.setProperty("--site-header-visible-height", Math.ceil(header.getBoundingClientRect().height) + "px");
            updateMenuGeometry();
        }

        function notifyNavigationState(reason) {
            window.dispatchEvent(new CustomEvent("paravane:mobile-nav-statechange", {
                detail: {
                    open: menuIsOpen(),
                    catalogOpen: Boolean(links.querySelector(".nav-catalog.is-open")),
                    reason: reason || "interaction"
                }
            }));
        }

        function setCatalogState(catalog, open) {
            catalog.classList.toggle("is-open", open);
            var trigger = catalog.querySelector(".nav-catalog-trigger");
            if (trigger) {
                trigger.setAttribute("aria-expanded", String(open));
            }
        }

        function closeCatalogs(except) {
            catalogs.forEach(function (catalog) {
                if (catalog !== except) {
                    setCatalogState(catalog, false);
                }
            });
        }

        function setCatalogOpen(catalog, open, restoreFocus) {
            if (open) {
                revealHeader();
                closeCatalogs(catalog);
            }

            setCatalogState(catalog, open);

            if (!open && restoreFocus) {
                catalog.querySelector(".nav-catalog-trigger").focus({ preventScroll: true });
            }

            notifyNavigationState();
        }

        function setMenuOpen(open, restoreFocus, reason) {
            open = Boolean(open && mobileQuery.matches);

            if (open) {
                menuOpenScrollY = documentScrollY();
                revealHeader();
            }

            /* Move focus before making the closed panel inert. Never scroll to it. */
            if (!open && (restoreFocus || links.contains(document.activeElement))) {
                button.focus({ preventScroll: true });
            }

            if (!open) {
                closeCatalogs();
            }

            header.classList.toggle("mobile-nav-open", open);
            root.classList.toggle("mobile-nav-is-open", open);
            button.setAttribute("aria-expanded", String(open));
            button.setAttribute("aria-label", open ? "Close navigation menu" : "Open navigation menu");
            links.inert = mobileQuery.matches && !open;

            if (!open) {
                links.scrollTop = 0;
            }

            updateMenuGeometry();
            notifyNavigationState(reason);
        }

        /*
         * This is a disclosure, not a modal: let the document scroll normally.
         * Listen only to document scrolling, not the panel's internal scroll,
         * so long catalogs remain usable. Once scrolling reaches the page,
         * dismiss the panel without cancelling the gesture or resetting Y.
         * A small threshold ignores subpixel/layout jitter while opening it.
         */
        document.addEventListener("scroll", function () {
            if (menuIsOpen() && Math.abs(documentScrollY() - menuOpenScrollY) >= scrollDismissTravel) {
                setMenuOpen(false, false, "scroll");
            }
        }, { passive: true });

        button.addEventListener("click", function (event) {
            var shouldOpen = !menuIsOpen();

            event.preventDefault();
            event.stopPropagation();
            setMenuOpen(shouldOpen, false, "toggle");
        });

        /* Products and Resources share one controller and one close/reset path. */
        catalogs.forEach(function (catalog, index) {
            var trigger = catalog.querySelector(".nav-catalog-trigger");
            var menu = catalog.querySelector(".product-catalog-menu");

            if (!trigger || !menu) {
                return;
            }

            if (!menu.id) {
                menu.id = "navigation-catalog-" + index;
            }

            trigger.setAttribute("aria-controls", menu.id);
            trigger.setAttribute("aria-expanded", "false");
            trigger.addEventListener("click", function (event) {
                if (!usesCatalogDisclosure(event)) {
                    return;
                }

                event.preventDefault();
                setCatalogOpen(catalog, trigger.getAttribute("aria-expanded") !== "true", false);
            });
        });

        header.addEventListener("click", function (event) {
            if (event.target.closest("a")) {
                setMenuOpen(false, false);
            }
        });

        document.addEventListener("click", function (event) {
            var eventPath = typeof event.composedPath === "function" ? event.composedPath() : [];
            var clickedInsideHeader = eventPath.length > 0 ? eventPath.indexOf(header) !== -1 : header.contains(event.target);

            if (menuIsOpen() && !clickedInsideHeader) {
                setMenuOpen(false, false);
                return;
            }

            var openCatalog = links.querySelector(".nav-catalog.is-open");
            if (openCatalog && !openCatalog.contains(event.target)) {
                setCatalogOpen(openCatalog, false, false);
            }
        });

        document.addEventListener("keydown", function (event) {
            if (event.key !== "Escape") {
                return;
            }

            var openCatalog = links.querySelector(".nav-catalog.is-open");
            if (openCatalog) {
                event.preventDefault();
                setCatalogOpen(openCatalog, false, true);
            } else if (menuIsOpen()) {
                event.preventDefault();
                setMenuOpen(false, true);
            }
        });

        /* A non-modal disclosure must not leave keyboard focus behind its panel. */
        document.addEventListener("focusin", function (event) {
            if (menuIsOpen() && !header.contains(event.target)) {
                setMenuOpen(false, false);
            }
        });

        function syncViewport() {
            updateMenuGeometry();

            if (!mobileQuery.matches && menuIsOpen()) {
                setMenuOpen(false, false);
            }

            if (!usesCatalogDisclosure()) {
                closeCatalogs();
            }

            links.inert = mobileQuery.matches && !menuIsOpen();
        }

        window.addEventListener("resize", syncViewport, { passive: true });
        if (window.visualViewport) {
            window.visualViewport.addEventListener("resize", updateMenuGeometry, { passive: true });
        }
        if (typeof ResizeObserver === "function") {
            new ResizeObserver(updateMenuGeometry).observe(header);
        }

        /* Do not restore a stale open accordion after back/forward navigation. */
        window.addEventListener("pageshow", function () {
            setMenuOpen(false, false);
        });

        updateMenuGeometry();
        links.inert = mobileQuery.matches;
    }

    if (document.readyState === "loading") {
        document.addEventListener("DOMContentLoaded", bindMobileNavigation, { once: true });
    } else {
        bindMobileNavigation();
    }

    function bindScrollAwareHeader() {
        var header = document.querySelector("body:not(.page-app) .site-header");

        if (!header || header.getAttribute("data-scroll-aware-bound") === "true") {
            return;
        }

        header.setAttribute("data-scroll-aware-bound", "true");

        var lastScrollY = readScrollY();
        var downwardTravel = 0;
        var headerHeight = 0;
        var frameRequested = false;
        var collapseTravel = 12;
        var topRevealPoint = 24;

        function readScrollY() {
            var scroller = document.scrollingElement || root;
            var maxScrollY = Math.max(0, scroller.scrollHeight - scroller.clientHeight);
            /* Ignore rubber-band overscroll at either end of a touch document. */
            return Math.min(Math.max(window.scrollY || 0, 0), maxScrollY);
        }

        function isHeaderInUse() {
            /*
             * Pointer clicks leave controls focused after activation. Treating
             * all focus as active use would pin the header open after clicking
             * the theme toggle. Preserve it only for visible keyboard focus or
             * while the product catalog is being used.
             */
            return header.classList.contains("mobile-nav-open") ||
                Boolean(header.querySelector(".nav-catalog.is-open")) ||
                Boolean(header.querySelector(":focus-visible:not([data-theme-toggle])")) ||
                (window.matchMedia("(hover: hover) and (pointer: fine)").matches &&
                    Boolean(header.querySelector(".nav-catalog:hover")));
        }

        function measureHeader() {
            headerHeight = Math.ceil(header.getBoundingClientRect().height);
            root.style.setProperty("--site-header-height", headerHeight + "px");

            if (!root.classList.contains("site-header-is-collapsed")) {
                root.style.setProperty("--site-header-visible-height", headerHeight + "px");
            }
        }

        function setCollapsed(collapsed) {
            if (collapsed && isHeaderInUse()) {
                collapsed = false;
            }

            header.classList.toggle("is-scroll-collapsed", collapsed);
            header.classList.toggle("is-scroll-expanded", !collapsed);
            root.classList.toggle("site-header-is-collapsed", collapsed);
            root.style.setProperty(
                "--site-header-visible-height",
                collapsed ? "0px" : headerHeight + "px"
            );
        }

        function updateHeader() {
            var currentScrollY = readScrollY();
            var delta = currentScrollY - lastScrollY;

            header.classList.toggle("is-scroll-elevated", currentScrollY > topRevealPoint);

            if (currentScrollY <= topRevealPoint || isHeaderInUse()) {
                downwardTravel = 0;
                setCollapsed(false);
            } else if (delta < 0) {
                downwardTravel = 0;
                setCollapsed(false);
            } else if (delta > 0) {
                downwardTravel += delta;

                if (downwardTravel >= collapseTravel && currentScrollY > headerHeight) {
                    setCollapsed(true);
                }
            }

            lastScrollY = currentScrollY;
            frameRequested = false;
        }

        function requestHeaderUpdate() {
            if (frameRequested) return;
            frameRequested = true;
            window.requestAnimationFrame(updateHeader);
        }

        measureHeader();
        setCollapsed(false);
        header.classList.toggle("is-scroll-elevated", lastScrollY > topRevealPoint);

        window.addEventListener("scroll", requestHeaderUpdate, { passive: true });
        window.addEventListener("resize", function () {
            measureHeader();
            requestHeaderUpdate();
        }, { passive: true });

        if (typeof ResizeObserver === "function") {
            new ResizeObserver(measureHeader).observe(header);
        }

        header.addEventListener("focusin", function () {
            downwardTravel = 0;
            setCollapsed(false);
        });

        window.addEventListener("paravane:mobile-nav-statechange", function (event) {
            measureHeader();

            /* Keep the current direction/travel when scrolling dismisses it. */
            if (event.detail && event.detail.reason === "scroll") {
                requestHeaderUpdate();
                return;
            }

            lastScrollY = readScrollY();
            downwardTravel = 0;
            setCollapsed(false);
        });

        window.addEventListener("paravane:themechange", function () {
            lastScrollY = readScrollY();
            downwardTravel = 0;
            measureHeader();
            setCollapsed(false);

            window.requestAnimationFrame(function () {
                measureHeader();
                requestHeaderUpdate();
            });
        });

        window.addEventListener("pageshow", function () {
            lastScrollY = readScrollY();
            downwardTravel = 0;
            measureHeader();
            setCollapsed(false);
        });
    }

    if (document.readyState === "loading") {
        document.addEventListener("DOMContentLoaded", bindThemeControls, { once: true });
        document.addEventListener("DOMContentLoaded", bindScrollAwareHeader, { once: true });
    } else {
        bindThemeControls();
        bindScrollAwareHeader();
    }

    window.addEventListener("pageshow", function (event) {
        /* Normal navigations already restored the theme before CSS loaded. */
        if (event.persisted) {
            restoreTheme();
            refreshSessionNavigation();
        } else {
            syncThemeUi(currentTheme());
        }
    });

    window.addEventListener("storage", function (event) {
        if (event.key === storageKey) {
            restoreTheme();
        }
    });

    window.ParavaneTheme = {
        isSwitchingEnabled: function () {
            return switchingEnabled;
        },
        get: currentTheme,
        set: setTheme,
        toggle: toggleTheme,
        restore: restoreTheme
    };
})();

/* Load the centralized campaign controller. It performs its own exact-route,
   session, authentication, and campaign-suppression eligibility checks. */
(function () {
    "use strict";

    if (document.querySelector('script[data-paravane-promotion-controller]')) return;

    var script = document.createElement("script");
    script.src = "../js/promotion.js?v=015.000.008-alpha.016-campaign-v1";
    script.async = false;
    script.setAttribute("data-paravane-promotion-controller", "");
    document.head.appendChild(script);
})();

/* ==========================================================================
   Desktop navigation catalog click lock

   A catalog opened by click remains the only hover-active catalog until the
   user clicks its trigger again, presses Escape, clicks outside, or selects a
   destination. Touch/mobile disclosure behavior remains managed above.
   ========================================================================== */
(function () {
    "use strict";

    function initNavCatalogClickLock() {
        var header = document.querySelector(".site-header");
        var hoverPointer = window.matchMedia("(hover: hover) and (pointer: fine)");
        var lockedCatalog = null;

        if (!header || header.dataset.catalogClickLockReady === "true") return;
        header.dataset.catalogClickLockReady = "true";

        function unlockCatalog() {
            if (lockedCatalog) lockedCatalog.classList.remove("is-click-locked");
            lockedCatalog = null;
            header.classList.remove("nav-catalog-click-locked");
        }

        function lockCatalog(catalog) {
            unlockCatalog();
            lockedCatalog = catalog;
            lockedCatalog.classList.add("is-click-locked");
            header.classList.add("nav-catalog-click-locked");
        }

        document.addEventListener("click", function (event) {
            var trigger;
            var catalog;
            var selectedLink;

            if (!hoverPointer.matches) {
                unlockCatalog();
                return;
            }

            trigger = event.target.closest ? event.target.closest(".nav-catalog-trigger") : null;
            if (trigger && header.contains(trigger)) {
                catalog = trigger.closest(".nav-catalog");
                if (!catalog) return;

                if (lockedCatalog && lockedCatalog !== catalog) {
                    event.preventDefault();
                    event.stopImmediatePropagation();
                    return;
                }

                if (lockedCatalog === catalog) {
                    unlockCatalog();
                    return;
                }

                lockCatalog(catalog);
                return;
            }

            if (!lockedCatalog) return;

            selectedLink = event.target.closest ? event.target.closest(".product-catalog-menu a") : null;
            if (selectedLink && lockedCatalog.contains(selectedLink)) {
                unlockCatalog();
                return;
            }

            if (!lockedCatalog.contains(event.target)) unlockCatalog();
        }, true);

        document.addEventListener("keydown", function (event) {
            if (event.key === "Escape") unlockCatalog();
        }, true);

        if (typeof hoverPointer.addEventListener === "function") {
            hoverPointer.addEventListener("change", function () {
                if (!hoverPointer.matches) unlockCatalog();
            });
        }
    }

    if (document.readyState === "loading") {
        document.addEventListener("DOMContentLoaded", initNavCatalogClickLock, { once: true });
    } else {
        initNavCatalogClickLock();
    }
})();

/* ==========================================================================
   Persistent navigation catalog current-page state

   Highlights the most specific Products or Resources dropdown destination
   that matches the current non-homepage URL, then preserves the corresponding
   top-level catalog trigger highlight for the duration of the page visit.
   ========================================================================== */
(function () {
    "use strict";

    function normalizePath(pathname) {
        var path = pathname || "/";

        try {
            path = decodeURIComponent(path);
        } catch (err) {}

        path = path
            .replace(/\/index\.html$/i, "")
            .replace(/\.html$/i, "")
            .replace(/\/+$/, "");

        return path || "/";
    }

    function routeMatches(currentPath, targetPath) {
        if (targetPath === "/") return currentPath === "/";
        return currentPath === targetPath || currentPath.indexOf(targetPath + "/") === 0;
    }

    function initCurrentCatalogDestination() {
        var header = document.querySelector(".site-header");
        var currentPath = normalizePath(window.location.pathname);
        var matches = [];

        if (!header || currentPath === "/") return;

        header.querySelectorAll(".product-catalog-menu a[href]").forEach(function (link) {
            var targetUrl;
            var targetPath;

            link.removeAttribute("aria-current");
            link.classList.remove("is-active");

            try {
                targetUrl = new URL(link.href, window.location.href);
            } catch (err) {
                return;
            }

            if (targetUrl.origin !== window.location.origin) return;

            targetPath = normalizePath(targetUrl.pathname);
            if (routeMatches(currentPath, targetPath)) {
                matches.push({ link: link, pathLength: targetPath.length });
            }
        });

        if (!matches.length) return;

        matches.sort(function (left, right) {
            return right.pathLength - left.pathLength;
        });

        var currentLink = matches[0].link;
        var currentCatalog = currentLink.closest(".nav-catalog");
        var currentTrigger = currentCatalog && currentCatalog.querySelector(".nav-catalog-trigger");

        currentLink.setAttribute("aria-current", "page");
        currentLink.classList.add("is-active");

        if (currentTrigger) {
            currentTrigger.classList.add("active");
        }
    }

    if (document.readyState === "loading") {
        document.addEventListener("DOMContentLoaded", initCurrentCatalogDestination, { once: true });
    } else {
        initCurrentCatalogDestination();
    }
})();
