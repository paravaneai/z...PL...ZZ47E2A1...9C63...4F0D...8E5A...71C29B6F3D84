(function () {
    ...
    function compensationCopy(salary) {
        return [
            ...
            "<p>The annual base salary range for this role is <strong>" + esc(salary.display) + "</strong>. Individual compensation within this range will vary based on the factors above and other considerations permitted by applicable law.</p>"
        ].join("");
    }

    function evergreenJobSections(salary) {
        return section("Compensation", compensationCopy(salary)) +
            section("Equal Opportunity & Inclusion", [
    ...
    function renderDetail(root, jobs) {
        ...
        copy.innerHTML = section("About Paravane", paragraphs(job.aboutParavane)) + section("About the role", '<p class="careers-job-summary">' + esc(job.summary) + "</p>" + paragraphs(job.description)) + section("What you'll do", bullets(job.responsibilities)) + section("What you bring", bullets(job.qualifications)) + (salary ? evergreenJobSections(salary) : "");
