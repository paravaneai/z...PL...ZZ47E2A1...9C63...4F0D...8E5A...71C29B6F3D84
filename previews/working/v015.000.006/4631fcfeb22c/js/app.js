(function () {
    ...
    var apiKeyItems = [];
    var apiKeyFilter = "active";
    var usageEventsState = { items: [], profile: "all", page: 1, pageSize: 10 };
    ...
    function startRouteLoaderAnimation() {
        var frameNode = $(".app-route-svg-frame");
    ...
    function readRouteTransitionTiming() {
        return {
            minimumDisplayMs: Math.min(transitionMilliseconds("--route-transition-min-display-ms", 420), 520),
            removalTransitionMs: Math.min(transitionMilliseconds("--route-transition-overlay-ms", 240), 280),
            navigationDelayMs: Math.max(Math.min(transitionMilliseconds("--route-transition-navigation-delay-ms", 240), 280), routeLoaderFrameDurationMs * 2)
        };
    }
    ...
    function setOptionalText(selector, value, root) {
        var el = $(selector, root);
        if (!el) return;
    ...
    function setStatus(message, state) {
        ...
        if (state && message) el.classList.add("is-" + state);
    }
    ...
    function showAppTransition() {
        ...
        $all(".app-route-backdrop, .app-route-loader").forEach(function (el) {
            el.hidden = false;
            el.setAttribute("aria-hidden", el.classList.contains("app-route-loader") ? "false" : "true");
            el.style.opacity = "1";
    ...
    function beginAppNavigation(url) {
        ...
        if (isPlatformPathname(url.pathname)) {
            document.documentElement.classList.remove("app-is-ready");
            document.documentElement.classList.add("app-is-leaving");
            window.setTimeout(function () {
    ...
    function redirectToLogin() {
        ...
        var next = (window.location.pathname || "../pages/app/") + window.location.search;
        var loginUrl = new URL("../pages/auth/signin", window.location.href);
    ...
    function updateUsageProgress(progressSelector, fillSelector, percentSelector, summarySelector, total, limit) {
        var progress = $(progressSelector);
        var fill = $(fillSelector);
        var percentLabel = $(percentSelector);
        var summary = $(summarySelector);
        var percentage = limit && isFinite(limit) ? Math.min(100, Math.max(0, (total / limit) * 100)) : 0;
        var rounded = Math.round(percentage);
        if (progress) progress.setAttribute("aria-valuenow", String(rounded));
        if (fill) fill.style.width = rounded + "%";
        if (percentLabel) percentLabel.textContent = limit ? rounded + "% used" : "Custom limit";
        if (summary) summary.textContent = limit
            ? formatNumber(total) + " of " + formatNumber(limit) + " credits used · " + formatNumber(Math.max(limit - total, 0)) + " remaining"
            : formatNumber(total) + " credits used in the current billing period";
    }
    ...
    function renderDashboard(data) {
        ...
        renderUsageRows((usage.recent || []).slice(0, 5));
        updateUsageProgress(
            "[data-dashboard-usage-progress]",
            "[data-dashboard-usage-progress-fill]",
            "[data-dashboard-usage-percent]",
            "[data-dashboard-usage-summary]",
            Number(usage.month_to_date || 0),
            usage.monthly_limit == null ? null : Number(usage.monthly_limit)
        );
        clearMetricLoading();
    ...
    function renderUsageEventPage() {
        var filtered = usageEventsState.items.filter(function (item) {
            return usageEventsState.profile === "all" || String(item.model_profile || "smtpRS") === usageEventsState.profile;
        });
        var totalPages = Math.max(1, Math.ceil(filtered.length / usageEventsState.pageSize));
        usageEventsState.page = Math.min(Math.max(usageEventsState.page, 1), totalPages);
        var start = (usageEventsState.page - 1) * usageEventsState.pageSize;
        renderUsageRows(filtered.slice(start, start + usageEventsState.pageSize));
        var pagination = $("[data-usage-pagination]");
        var summary = $("[data-usage-page-summary]");
        var previous = $("[data-usage-prev]");
        var next = $("[data-usage-next]");
        if (pagination) pagination.hidden = filtered.length <= usageEventsState.pageSize;
        if (summary) summary.textContent = filtered.length ? "Showing " + (start + 1) + "–" + Math.min(start + usageEventsState.pageSize, filtered.length) + " of " + filtered.length + " events" : "No events match this filter.";
        if (previous) previous.disabled = usageEventsState.page <= 1;
        if (next) next.disabled = usageEventsState.page >= totalPages;
    }

    function initUsageEventControls(items) {
        usageEventsState.items = Array.isArray(items) ? items : [];
        usageEventsState.page = 1;
        var filter = $("[data-usage-profile-filter]");
        if (filter) {
            var profiles = usageEventsState.items.map(function (item) { return String(item.model_profile || "smtpRS"); })
                .filter(function (value, index, values) { return values.indexOf(value) === index; }).sort();
            filter.innerHTML = '<option value="all">All profiles</option>' + profiles.map(function (profile) {
                return '<option value="' + escapeHtml(profile) + '">' + escapeHtml(profile) + '</option>';
            }).join("");
            if (!filter.dataset.bound) {
                filter.dataset.bound = "true";
                filter.addEventListener("change", function () { usageEventsState.profile = filter.value; usageEventsState.page = 1; renderUsageEventPage(); });
            }
        }
        var previous = $("[data-usage-prev]");
        var next = $("[data-usage-next]");
        if (previous && !previous.dataset.bound) { previous.dataset.bound = "true"; previous.addEventListener("click", function () { usageEventsState.page -= 1; renderUsageEventPage(); }); }
        if (next && !next.dataset.bound) { next.dataset.bound = "true"; next.addEventListener("click", function () { usageEventsState.page += 1; renderUsageEventPage(); }); }
        renderUsageEventPage();
    }
    ...
    async function initUsagePage() {
        ...
        try {
            ...
            initUsageEventControls(usage.recent);
            updateUsageProgress("[data-usage-progress]", "[data-usage-progress-fill]", "[data-usage-percent]", "[data-usage-progress-summary]", total, limit);
            clearMetricLoading();
            setStatus("", "success");
        } catch (err) {
            clearMetricLoading();
    ...
    function updateApiKeyCreateState(data) {
        ...
        var limitText = limit === null ? "Custom allowance" : activeCount + " active of " + limit + " allowed";
    ...
    function renderApiKeyRows(items) {
        ...
        if (!body) return;
        if (Array.isArray(items)) apiKeyItems = items;
        var rows = apiKeyItems.filter(function (item) {
            if (apiKeyFilter === "all") return true;
            return apiKeyFilter === "active" ? isActiveApiKey(item) : !isActiveApiKey(item);
        });
        apiKeyRowsByRef = {};
        body.innerHTML = rows.map(function (item, index) {
            ...
            return [
                ...
                '<td><span class="app-state-badge ' + (active ? "is-active" : "is-revoked") + '">' + (active ? "Active" : "Revoked") + "</span></td>",
                "<td>" + escapeHtml(formatDate(item.created_at)) + "</td>",
        ...
        if (empty) {
            empty.textContent = apiKeyFilter === "active" ? "No active API keys are available for this workspace." : apiKeyFilter === "revoked" ? "No revoked API keys are available for this workspace." : "No API keys have been created for this workspace.";
            empty.hidden = rows.length > 0;
        }
    }

    function initApiKeyFilter() {
        var filter = $("[data-api-key-filter]");
        if (!filter) return;
        apiKeyFilter = filter.value || "active";
        filter.addEventListener("change", function () { apiKeyFilter = filter.value; renderApiKeyRows(); });
    }
    ...
    function renderBilling(data) {
        ...
        setOptionalText("[data-cancel-status]", sub && sub.cancel_at_period_end ? "Cancels at period end" : "");
    ...
    async function initBillingPage() {
        ...
        try {
            ...
            setOptionalText("[data-cancel-status]", "");
    ...
    function initAccountDeletion() {
        ...
        form.addEventListener("submit", async function (event) {
            ...
            try {
                ...
                showAppTransition();
                var loginUrl = new URL("../pages/auth/signin?account=deleted", window.location.href);
                writePageArrivalState(loginUrl);
    ...
    function portalInitials() {
        var user = (portalSession && portalSession.user) || {};
        var source = String(user.name || user.email || "Account").trim();
    ...
    function initAccountMenu() {
        var logoutButton = $(".app-site-header [data-logout]");
        if (!logoutButton || logoutButton.closest(".app-account-menu")) return;
        var user = (portalSession && portalSession.user) || {};
        ...
        menu.innerHTML = [
            ...
            '<div class="app-account-panel" role="menu" hidden><div class="app-account-identity"><strong>' + escapeHtml(user.name || "Customer account") + '</strong><small>' + escapeHtml(user.email || "") + '</small></div><a href="../pages/app/" role="menuitem"><i class="fa-solid fa-gauge-high" aria-hidden="true"></i><span>Dashboard</span></a><a href="../pages/app/settings" role="menuitem"><i class="fa-solid fa-gear" aria-hidden="true"></i><span>Account settings</span></a><a href="../pages/contact/" role="menuitem"><i class="fa-regular fa-circle-question" aria-hidden="true"></i><span>Support</span></a></div>'
        ].join("");
        logoutButton.parentNode.insertBefore(menu, logoutButton);
        var panel = $(".app-account-panel", menu);
        var trigger = $(".app-account-trigger", menu);
        logoutButton.className = "app-account-menu-action";
        logoutButton.innerHTML = '<i class="fa-solid fa-arrow-right-from-bracket" aria-hidden="true"></i><span>Logout</span>';
        logoutButton.setAttribute("role", "menuitem");
        panel.appendChild(logoutButton);
        function closeMenu() { panel.hidden = true; trigger.setAttribute("aria-expanded", "false"); menu.classList.remove("is-open"); }
        trigger.addEventListener("click", function () { var open = panel.hidden; closeMenu(); if (open) { panel.hidden = false; trigger.setAttribute("aria-expanded", "true"); menu.classList.add("is-open"); } });
        document.addEventListener("click", function (event) { if (!menu.contains(event.target)) closeMenu(); });
        document.addEventListener("keydown", function (event) { if (event.key === "Escape" && !panel.hidden) { closeMenu(); trigger.focus(); } });
    }

    function initMobileAccountNav() {
        var sidebar = $(".app-sidebar");
        var active = sidebar ? $(".app-nav-link.active", sidebar) : null;
        if (!sidebar || !active || $("[data-app-sidebar-toggle]", sidebar)) return;
        var button = document.createElement("button");
        button.type = "button";
        button.className = "app-sidebar-mobile-toggle";
        button.setAttribute("data-app-sidebar-toggle", "");
        button.setAttribute("aria-expanded", "false");
        button.innerHTML = '<span><small>Account</small><strong>' + escapeHtml(active.textContent.trim()) + '</strong></span><i class="fa-solid fa-chevron-down" aria-hidden="true"></i>';
        sidebar.insertBefore(button, sidebar.firstChild);
        button.addEventListener("click", function () { var open = sidebar.classList.toggle("is-open"); button.setAttribute("aria-expanded", String(open)); });
    }

    function setSettingsMessage(target, message, state) {
        if (!target) return;
        target.textContent = message || "";
        target.classList.remove("is-success", "is-error", "is-loading");
        if (state && message) target.classList.add("is-" + state);
    }

    function setSettingsBusy(form, button, busy, busyLabel) {
        if (!form || !button) return;
        form.classList.toggle("is-busy", !!busy);
        button.setAttribute("aria-busy", busy ? "true" : "false");
        if (!button.dataset.idleHtml) button.dataset.idleHtml = button.innerHTML;
        button.innerHTML = busy
            ? '<span class="app-settings-button-spinner" aria-hidden="true"></span><span>' + escapeHtml(busyLabel || "Saving") + '</span>'
            : button.dataset.idleHtml;
        if (busy) button.disabled = true;
    }

    function setSettingsStateBadge(target, status) {
        if (!target) return;
        var normalized = String(status || "active").toLowerCase();
        target.textContent = formatLabel(status || "active");
        target.classList.toggle("is-active", normalized === "active");
        target.classList.toggle("is-revoked", normalized !== "active");
    }

    function refreshAccountIdentityUi() {
        var user = (portalSession && portalSession.user) || {};
        var identityName = user.name || "Customer account";
        $all(".app-account-avatar, [data-settings-avatar]").forEach(function (avatar) {
            avatar.textContent = portalInitials();
        });
        var menuIdentity = $(".app-account-identity strong");
        if (menuIdentity) menuIdentity.textContent = identityName;
        setText("[data-settings-overview-name]", identityName);
    }

    function accountUpdateUnavailableMessage(err) {
        if (err && (err.status === 404 || err.status === 405)) {
            return "Account editing is not enabled on this API deployment yet. The dashboard control is ready, but PATCH /v1/app/account must be enabled before this change can be saved.";
        }
        return (err && err.message) || "This account change could not be saved. Please try again.";
    }

    function initAccountSettings() {
        var page = $("[data-account-settings]");
        if (!page) return;

        var user = (portalSession && portalSession.user) || {};
        var tenant = (portalSession && portalSession.tenant) || {};
        var profileForm = $("[data-settings-profile-form]", page);
        var profileInput = $("[data-settings-name-input]", page);
        var profileButton = $("[data-settings-profile-save]", page);
        var profileMessage = $("[data-settings-profile-status]", page);
        var workspaceForm = $("[data-settings-workspace-form]", page);
        var workspaceInput = $("[data-settings-workspace-input]", page);
        var workspaceButton = $("[data-settings-workspace-save]", page);
        var workspaceMessage = $("[data-settings-workspace-message]", page);
        var passwordForm = $("[data-settings-password-form]", page);
        var passwordButton = $("[data-settings-password-button]", page);
        var passwordMessage = $("[data-settings-password-status]", page);
        var initialName = String(user.name || "").trim();
        var initialWorkspaceName = String(tenant.name || "").trim();

        function syncProfileButton() {
            if (!profileInput || !profileButton) return;
            var nextValue = profileInput.value.trim();
            profileButton.disabled = !nextValue || nextValue === initialName || profileForm.classList.contains("is-busy");
        }

        function syncWorkspaceButton() {
            if (!workspaceInput || !workspaceButton) return;
            var nextValue = workspaceInput.value.trim();
            workspaceButton.disabled = !nextValue || nextValue === initialWorkspaceName || workspaceForm.classList.contains("is-busy");
        }

        if (profileInput) {
            profileInput.value = initialName;
            profileInput.placeholder = initialName ? "Full name" : "Add your name";
        }
        if (workspaceInput) workspaceInput.value = initialWorkspaceName;
        setText("[data-settings-user-email]", user.email, page);
        setText("[data-settings-profile-email]", user.email, page);
        setText("[data-settings-password-email]", user.email, page);
        setText("[data-settings-overview-workspace]", tenant.name, page);
        setText("[data-settings-plan]", planLabel(tenant.plan || "FREE"), page);
        setText("[data-settings-workspace-plan]", planLabel(tenant.plan || "FREE"), page);
        setSettingsStateBadge($("[data-settings-user-status]", page), user.status || "active");
        setSettingsStateBadge($("[data-settings-tenant-status]", page), tenant.status || "active");
        setSettingsStateBadge($("[data-settings-workspace-status]", page), tenant.status || "active");
        refreshAccountIdentityUi();
        syncProfileButton();
        syncWorkspaceButton();

        if (profileInput) {
            profileInput.addEventListener("input", function () {
                setSettingsMessage(profileMessage, "", null);
                syncProfileButton();
            });
        }
        ...
        if (workspaceInput) {
            workspaceInput.addEventListener("input", function () {
                setSettingsMessage(workspaceMessage, "", null);
                syncWorkspaceButton();
            });
        }
        ...
        if (profileForm) {
            profileForm.addEventListener("submit", async function (event) {
                ...
                setSettingsMessage(profileMessage, "Saving your name...", "loading");
                setSettingsBusy(profileForm, profileButton, true, "Saving");
                try {
                    var result = await requestJson("../v1/app/account", {
                        method: "PATCH",
                        headers: { "Content-Type": "application/json" },
                        body: JSON.stringify({ name: name })
                    });
                    portalSession.user = Object.assign({}, portalSession.user || {}, result.user || {}, { name: ((result.user && result.user.name) || result.name || name) });
                    initialName = String(portalSession.user.name || name).trim();
                    profileInput.value = initialName;
                    refreshAccountIdentityUi();
                    setSettingsMessage(profileMessage, "Name updated.", "success");
                    setStatus("Name updated.", "success");
                } catch (err) {
                    setSettingsMessage(profileMessage, accountUpdateUnavailableMessage(err), "error");
                    setStatus("Name was not updated.", "error");
                } finally {
                    setSettingsBusy(profileForm, profileButton, false);
                    syncProfileButton();
                }
            });
        }
        ...
        if (workspaceForm) {
            workspaceForm.addEventListener("submit", async function (event) {
                event.preventDefault();
                if (!workspaceForm.checkValidity()) {
                    workspaceForm.reportValidity();
                    return;
                }
                var workspaceName = workspaceInput.value.trim();
                if (!workspaceName || workspaceName === initialWorkspaceName) {
                    syncWorkspaceButton();
                    return;
                }
                setSettingsMessage(workspaceMessage, "Saving workspace name...", "loading");
                setSettingsBusy(workspaceForm, workspaceButton, true, "Saving");
                try {
                    var result = await requestJson("../v1/app/account", {
                        method: "PATCH",
                        headers: { "Content-Type": "application/json" },
                    ...
                    portalSession.tenant = Object.assign({}, portalSession.tenant || {}, result.tenant || {}, { name: ((result.tenant && result.tenant.name) || result.workspace_name || workspaceName) });
                    initialWorkspaceName = String(portalSession.tenant.name || workspaceName).trim();
                    workspaceInput.value = initialWorkspaceName;
                    setText("[data-settings-overview-workspace]", initialWorkspaceName, page);
                    setSettingsMessage(workspaceMessage, "Workspace name updated.", "success");
                    setStatus("Workspace name updated.", "success");
                } catch (err) {
                    setSettingsMessage(workspaceMessage, accountUpdateUnavailableMessage(err), "error");
                    setStatus("Workspace name was not updated.", "error");
                } finally {
                    setSettingsBusy(workspaceForm, workspaceButton, false);
                    syncWorkspaceButton();
                }
            });
        }
        ...
        if (passwordForm && passwordButton) {
            passwordForm.addEventListener("submit", async function (event) {
                event.preventDefault();
                var email = String(((portalSession && portalSession.user) || {}).email || "").trim();
                if (!email) {
                    setSettingsMessage(passwordMessage, "Your account email is unavailable. Contact support to change your password.", "error");
                    return;
                }
                setSettingsMessage(passwordMessage, "Requesting a secure password-change link...", "loading");
                setSettingsBusy(passwordForm, passwordButton, true, "Sending");
                try {
                    ...
                    setSettingsMessage(passwordMessage, "Password-change link sent. Check your email to choose a new password.", "success");
                    setStatus("Password-change link sent.", "success");
                } catch (err) {
                    setSettingsMessage(passwordMessage, (err && err.message) || "A password-change link could not be sent. Please try again.", "error");
                    setStatus("Password-change request failed.", "error");
                } finally {
                    setSettingsBusy(passwordForm, passwordButton, false);
    ...
    async function initProtectedApp() {
        var protectedPage = $("[data-app-dashboard]") || $("[data-usage-page]") ||
            $("[data-api-keys-page]") || $("[data-billing-page]") || $("[data-account-settings]");
        if (!protectedPage) {
            hideAppTransition();
            return;
        }
        if (!await requirePortalAccess()) return;
