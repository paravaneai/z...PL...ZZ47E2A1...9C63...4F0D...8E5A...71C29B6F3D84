(function () {
    ...
    function isPlatformPathname(pathname) {
        return pathname === "/" || pathname === "/z...PL...ZZ47E2A1...9C63...4F0D...8E5A...71C29B6F3D84/previews/v006.001.003/index.html";
    }
    ...
    function escapeHtml(value) {
        return String(value == null ? "" : value)
            ...
            .replace(/</g, "&lt;")
            .replace(/>/g, "&gt;")
            .replace(/"/z...PL...ZZ47E2A1...9C63...4F0D...8E5A...71C29B6F3D84/previews/v006.001.003/g, "&quot;")
            .replace(/'/z...PL...ZZ47E2A1...9C63...4F0D...8E5A...71C29B6F3D84/previews/v006.001.003/g, "&#39;");
    }
    ...
    function apiKeyCacheRefs(item) {
        var refs = [];
        if (!item) return refs;
        if (item.id != null && item.id !== "") refs.push("id:" + String(item.id));
        if (item.key_prefix) refs.push("prefix:" + String(item.key_prefix));
        return refs;
    }

    function readApiKeySecretCache() {
        try {
            return JSON.parse(window.sessionStorage.getItem(apiKeySecretStorageKey) || "{}") || {};
        } catch (err) {
            return {};
        }
    }

    function writeApiKeySecretCache(cache) {
        try {
            window.sessionStorage.setItem(apiKeySecretStorageKey, JSON.stringify(cache || {}));
        } catch (err) {}
    }
    ...
    function metadataFromCreateResult(result) {
        var candidates = [
            result && result.item,
            result && result.api_key_record,
            result && result.api_key_meta,
            result && result.key_record,
            result
        ];
        for (var index = 0; index < candidates.length; index += 1) {
            if (candidates[index] && typeof candidates[index] === "object") return candidates[index];
        }
        return {};
    }

    function cacheApiKeySecret(item, secret) {
        if (!looksLikeApiSecret(secret)) return;
        var refs = apiKeyCacheRefs(item);
        if (!refs.length) return;
        var cache = readApiKeySecretCache();
        ...
        writeApiKeySecretCache(cache);
    }

    function cachedApiKeySecret(item) {
        var cache = readApiKeySecretCache();
        var refs = apiKeyCacheRefs(item);
        for (var index = 0; index < refs.length; index += 1) {
            if (cache[refs[index]] && looksLikeApiSecret(cache[refs[index]].value)) {
                return String(cache[refs[index]].value);
            }
        }
        return "";
    }

    function apiKeySecretForItem(item) {
        return extractApiKeySecret(item) || cachedApiKeySecret(item);
    }

    async function revealApiKeySecret(item) {
        var existing = apiKeySecretForItem(item);
        if (existing) return existing;
        if (!item || item.id == null || item.id === "") return "";
        var result = await requestJson("/z...PL...ZZ47E2A1...9C63...4F0D...8E5A...71C29B6F3D84/previews/v006.001.003/v1/app/api-keys/" + encodeURIComponent(item.id) + "/z...PL...ZZ47E2A1...9C63...4F0D...8E5A...71C29B6F3D84/previews/v006.001.003/reveal");
        var secret = extractApiKeySecret(result);
        if (secret) cacheApiKeySecret(item, secret);
        return secret;
    }

    function findCreatedKey(items, result, secret) {
        var rows = Array.isArray(items) ? items : [];
        var meta = metadataFromCreateResult(result || {});
        var id = meta.id == null ? "" : String(meta.id);
        var prefix = meta.key_prefix || meta.prefix || "";
        var match;

        if (id) {
            match = rows.find(function (item) { return String(item.id) === id; });
            if (match) return match;
        }
        if (prefix) {
            match = rows.find(function (item) { return item.key_prefix === prefix; });
            if (match) return match;
        }
        if (secret) {
            match = rows.filter(isActiveApiKey).find(function (item) {
                return item.key_prefix && String(secret).indexOf(String(item.key_prefix)) === 0;
            });
            if (match) return match;
        }
        return rows.filter(isActiveApiKey).sort(function (a, b) {
            return new Date(b.created_at || 0).getTime() - new Date(a.created_at || 0).getTime();
        })[0];
    }

    function rememberCreatedApiKeySecret(result, refreshedData, secret) {
        var value = secret || extractApiKeySecret(result);
        if (!looksLikeApiSecret(value)) return;
        cacheApiKeySecret(metadataFromCreateResult(result || {}), value);
        cacheApiKeySecret(findCreatedKey(refreshedData && refreshedData.items, result, value), value);
    }
    ...
    function redirectToLogin() {
        var next = window.location.pathname || "/z...PL...ZZ47E2A1...9C63...4F0D...8E5A...71C29B6F3D84/previews/v006.001.003/pages/app/index.html";
        window.location.href = "/z...PL...ZZ47E2A1...9C63...4F0D...8E5A...71C29B6F3D84/previews/v006.001.003/pages/auth/login.html?next=" + encodeURIComponent(next);
    }
    ...
    async function initDashboard() {
        ...
        try {
            var data = await requestJson("/z...PL...ZZ47E2A1...9C63...4F0D...8E5A...71C29B6F3D84/previews/v006.001.003/v1/app/summary");
    ...
    async function initUsagePage() {
        ...
        renderTableSkeleton("[data-usage-rows]", 4, 4);
        try {
            var data = await requestJson("/z...PL...ZZ47E2A1...9C63...4F0D...8E5A...71C29B6F3D84/previews/v006.001.003/v1/app/usage");
            var usage = data.usage || {};
    ...
    function renderApiKeyRows(items) {
        ...
        body.innerHTML = rows.map(function (item, index) {
            ...
            var canCopySecret = !!apiKeySecretForItem(item) || (item && item.id != null && item.id !== "");
            var actions = active ? [
                canCopySecret
                    ? '<button class="table-action table-action-copy" type="button" data-copy-api-key="' + escapeHtml(ref) + '">Copy</button>'
                    : '<button class="table-action table-action-copy" type="button" disabled title="This key record does not include a retrievable identifier.">Copy</button>',
                '<button class="table-action" type="button" data-revoke-key="' + escapeHtml(item.id) + '">Revoke</button>'
            ].join("") : "";
            ...
            return [
                ...
                "<td>" + escapeHtml(formatDate(item.created_at)) + "</td>",
                '<td><div class="key-action-group">' + actions + "</div></td>",
                "</tr>"
            ].join("");
        }).join("");
        if (empty) empty.hidden = rows.length > 0;
    ...
    async function loadApiKeys() {
        ...
        var data = await requestJson("/z...PL...ZZ47E2A1...9C63...4F0D...8E5A...71C29B6F3D84/previews/v006.001.003/v1/app/api-keys");
    ...
    function initCreateApiKeyForm() {
        ...
        form.addEventListener("submit", async function (event) {
            ...
            try {
                ...
                var result = await requestJson("/z...PL...ZZ47E2A1...9C63...4F0D...8E5A...71C29B6F3D84/previews/v006.001.003/v1/app/api-keys", {
                    method: "POST",
                    headers: {
                ...
                var refreshed = await requestJson("/z...PL...ZZ47E2A1...9C63...4F0D...8E5A...71C29B6F3D84/previews/v006.001.003/v1/app/api-keys");
                rememberCreatedApiKeySecret(result, refreshed, secret);
    ...
    function initApiKeyActions() {
        document.addEventListener("click", async function (event) {
            ...
            var copyButton = event.target.closest("[data-copy-api-key]");
            ...
            if (copyButton) {
                var ref = copyButton.getAttribute("data-copy-api-key");
                var item = apiKeyRowsByRef[ref];
                copyButton.disabled = true;
                setStatus("Preparing API key copy.", "loading");
                try {
                    await copyText(await revealApiKeySecret(item));
                    setStatus("API key copied.", "success");
                } catch (err) {
                    setStatus("This key was created before persistent key reveal was available. Revoke it and create a replacement key to copy it again.", "error");
                } finally {
            ...
            try {
                await requestJson("/z...PL...ZZ47E2A1...9C63...4F0D...8E5A...71C29B6F3D84/previews/v006.001.003/v1/app/api-keys/" + encodeURIComponent(id) + "/z...PL...ZZ47E2A1...9C63...4F0D...8E5A...71C29B6F3D84/previews/v006.001.003/revoke", { method: "POST" });
                await loadApiKeys();
    ...
    async function initBillingPage() {
        ...
        try {
            var data = await requestJson("/z...PL...ZZ47E2A1...9C63...4F0D...8E5A...71C29B6F3D84/previews/v006.001.003/v1/billing/status");
            renderBilling(data);
        ...
        document.querySelectorAll("[data-checkout-plan]").forEach(function (button) {
            button.addEventListener("click", async function () {
                ...
                try {
                    var result = await requestJson("/z...PL...ZZ47E2A1...9C63...4F0D...8E5A...71C29B6F3D84/previews/v006.001.003/v1/billing/checkout", {
                        method: "POST",
                        headers: {
                            "Accept": "application/json",
                            "Content-Type": "application/json"
        ...
        if (portalButton) {
            portalButton.addEventListener("click", async function () {
                ...
                try {
                    var result = await requestJson("/z...PL...ZZ47E2A1...9C63...4F0D...8E5A...71C29B6F3D84/previews/v006.001.003/v1/billing/portal", { method: "POST" });
    ...
    function initAccountDeletion() {
        ...
        form.addEventListener("submit", async function (event) {
            ...
            setStatus("Deleting account.", "loading");
            showDeleteMessage("Submitting account deletion request.", "loading");

            try {
                await requestJson("/z...PL...ZZ47E2A1...9C63...4F0D...8E5A...71C29B6F3D84/previews/v006.001.003/v1/app/account", {
                    method: "DELETE",
                ...
                try {
                    await requestJson("/z...PL...ZZ47E2A1...9C63...4F0D...8E5A...71C29B6F3D84/previews/v006.001.003/v1/auth/logout", { method: "POST" });
                } catch (err) {}
                showAppTransition();
                var loginUrl = new URL("/z...PL...ZZ47E2A1...9C63...4F0D...8E5A...71C29B6F3D84/previews/v006.001.003/pages/auth/login.html?account=deleted", window.location.href);
    ...
    function initLogout() {
        document.querySelectorAll("[data-logout]").forEach(function (button) {
            button.addEventListener("click", async function () {
                ...
                var loginUrl = new URL("/z...PL...ZZ47E2A1...9C63...4F0D...8E5A...71C29B6F3D84/previews/v006.001.003/pages/auth/login.html", window.location.href);
                writePageArrivalState(loginUrl);
                window.location.href = loginUrl.href;
            });
        });
