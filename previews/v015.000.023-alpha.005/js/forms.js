(function () {
    ...
    function pageTheme(form) {
        var preview = form && form.closest && form.closest(".smtprs-studio[data-preview-theme]");
        if (preview) return preview.dataset.previewTheme === "light" ? "light" : "dark";
    ...
    // Display feedback is independent of the request and the persistent results panel.
    // Timings/ripple follow ValidationActionButton.cs v002.012.002.
    var demoActionRuns = new WeakMap();
    function demoActionButton(form) { return form.querySelector(".smtprs-action-button"); }
    function demoActionRun(form) {
        var run = demoActionRuns.get(form);
        if (!run) { run = { timer: null, ripple: null }; demoActionRuns.set(form, run); }
        return run;
    }
    function setDemoAction(form, state, status) {
        var button = demoActionButton(form);
        if (!button) return;
        var run = demoActionRun(form);
        window.clearTimeout(run.timer);
        run.timer = null;
        button.dataset.state = state;
        // Like the .NET command, only the live operation blocks another activation.
        button.disabled = state === "running";
        button.setAttribute("aria-busy", String(state === "running"));
        var label = button.querySelector(".smtprs-action-label");
        if (label) label.textContent = state === "error" && status === "Invalid email address" ? "Check address again" : "Analyze";
        var text = status || ({idle: "Ready", running: "Validating…", success: "Low public risk", warning: "Review result", error: "Failed"})[state];
        button.querySelector(".smtprs-action-status").textContent = text;
    }
    function finishDemoAction(form, state, status) {
        var button = demoActionButton(form);
        if (!button || !button.isConnected) return;
        setDemoAction(form, state, status);
        var run = demoActionRun(form);
        // A fresh request cancels this feedback timer. Never delay a result or
        // let an old completion callback power down a newer running operation.
        run.timer = window.setTimeout(function () {
            run.timer = null;
            if (button.isConnected && button.dataset.state === state && !form.classList.contains("is-busy")) {
                setDemoAction(form, "idle");
            }
        }, state === "success" ? 700 : 850);
    }
    function playDemoActionRipple(form) {
        var button = demoActionButton(form);
        if (!button || window.matchMedia("(prefers-reduced-motion: reduce)").matches) return;
        var ripple = button.querySelector(".smtprs-action-ripple");
        if (!ripple || typeof ripple.animate !== "function") return;
        var run = demoActionRun(form);
        if (run.ripple) run.ripple.cancel();
        var animation = ripple.animate([
            { opacity: .32, transform: "scale(.55)" },
            { opacity: 0, transform: "scale(1.15)" }
        ], { duration: 480, easing: "linear" });
        run.ripple = animation;
        animation.finished.then(function () {
            if (run.ripple === animation) run.ripple = null;
        }).catch(function () { /* Superseded by a new click, motion preference, or view removal. */ });
    }
    function bindDemoAction(form) {
        var button = demoActionButton(form);
        if (!button) return;
        var motion = window.matchMedia("(prefers-reduced-motion: reduce)");
        function cancelRipple() {
            var run = demoActionRuns.get(form);
            if (run && run.ripple) { run.ripple.cancel(); run.ripple = null; }
        }
        function updateMotion() { if (motion.matches) cancelRipple(); }
        function release() {
            var run = demoActionRuns.get(form);
            if (run) window.clearTimeout(run.timer);
            cancelRipple();
            if (button.dataset.state !== "running") setDemoAction(form, "idle");
        }
        function syncDisabled() {
            if (button.dataset.state === "idle") button.querySelector(".smtprs-action-status").textContent = button.disabled ? "Unavailable" : "Ready";
        }
        var disabledObserver = new MutationObserver(syncDisabled);
        disabledObserver.observe(button, { attributes: true, attributeFilter: ["disabled"] });
        // Removing/recreating a view must not replay its old completion flash.
    ...
    function demoActionResultState(result) {
        var signals = result.signals || {};
        // Unchecked mailboxes are expected in this preview. Missing/uncertain public
        // evidence must not turn green merely because the HTTP request succeeded.
        return result.risk_band === "low" &&
            signals.address_risk && signals.address_risk.status === "clear" &&
            signals.domain_mail_capability && signals.domain_mail_capability.status === "capable"
            ? "success" : "warning";
    }
    ...
    function setBusy(form, busy) {
        ...
        if (button && button.classList.contains("smtprs-action-button")) {
            if (busy) setDemoAction(form, "running");
    ...
    function reportFirstInvalidField(form) {
        ...
        if (invalid) {
            if (form.dataset.apiForm === "demo" && invalid.name === "email") {
                form.dataset.invalidEmail = "true";
                form.dataset.preserveFormError = "true";
                invalid.setAttribute("aria-invalid", "true");
                setStatus(form, "Enter a valid email address before running analysis. No API request was sent.", "error");
                invalid.focus();
            } else {
        ...
        // A recent-result selection or autofill can update the value without input.
        if (form.dataset.invalidEmail === "true") {
            var email = form.querySelector('input[name="email"]');
            if (email) email.removeAttribute("aria-invalid");
            delete form.dataset.invalidEmail;
    ...
    function rerenderTurnstileForTheme(event) {
        if (!window.turnstile) return;
        var localChange = event && event.type === "smtprs:themechange";
        var scope = localChange ? event.target : document;
        scope.querySelectorAll("form[data-api-form]").forEach(function (form) {
            // A site-wide theme change must not reset a separately themed preview.
            if (!localChange && form.closest(".smtprs-studio[data-preview-theme]")) return;
    ...
    // Optional page-specific presentation; the shared transport stays authoritative.
    function notifyDemoWorkspace(form, name, detail, cancelable) {
        if (typeof form.dispatchEvent !== "function") return false;
        return !form.dispatchEvent(new CustomEvent("smtprs:" + name, {
            detail: detail, cancelable: Boolean(cancelable)
        }));
    }
    ...
    function setDemoState(form, state, checkedText) {
        ...
        notifyDemoWorkspace(form, "state", { state: state });
    }
    ...
    function renderDemoResult(form, result, requestContext) {
        var el = demoResultEl(form);
        if (!el) return;
        var completedAt = new Date().toISOString();
        setDemoState(form, "complete", "Checked " + completedAt.slice(11, 19) + " UTC");
        if (notifyDemoWorkspace(form, "result", {
            result: result, email: requestContext ? requestContext.email : "", completedAt: completedAt
        }, true)) return;
    ...
    async function submitForm(form) {
        ...
        if (!endpoint || form.classList.contains("is-busy") || (demoActionButton(form) && demoActionButton(form).disabled)) return;
        ...
        try {
            ...
            var demoRequestContext = kind === "demo" ? { email: JSON.parse(requestOptions.body).email } : null;
            ...
            if (kind === "demo") {
                ...
                renderDemoResult(form, result, demoRequestContext);
                actionResultState = demoActionResultState(result);
                setStatus(form, form.closest && form.closest(".smtprs-studio")
                    ? "Analysis complete. Select a signal to inspect its evidence."
                    : "Analysis complete. Expand a signal card to inspect its evidence.", "success");
            } else {
            ...
            // Always release the request and finish the control, even if verification reset fails.
            try { resetTurnstile(form); } finally {
                setBusy(form, false);
                if (kind === "demo") finishDemoAction(form, actionResultState);
            }
        }
    }
    ...
    function bindApiForm(form) {
        ...
        if (form.dataset.apiForm === "demo") {
            bindDemoAction(form);
            // Route click and Enter through the same validation and inline feedback.
            // Constraints remain on the input; other site forms retain native validation.
            form.noValidate = true;
            var email = form.querySelector('input[name="email"]');
            function clearEmailError() {
                if (form.dataset.invalidEmail !== "true") return;
                delete form.dataset.invalidEmail;
                delete form.dataset.preserveFormError;
                email.removeAttribute("aria-invalid");
                setDemoAction(form, "idle");
                setStatus(form, "Enter an email address when you are ready.", null);
            }
            email.addEventListener("input", function () {
                if (email.validity.valid) clearEmailError();
            });
            form.addEventListener("smtprs:reset", function () {
                ...
                delete form.dataset.preserveFormError;
                setDemoAction(form, "idle");
                setDemoState(form, "ready", "Awaiting a check");
                setStatus(form, "Enter an email address when you are ready.", null);
            });
        }
        renderTurnstile(form).catch(function () {});
        ...
        function attemptSubmit(event) {
            event.preventDefault();
            if (form.classList.contains("is-busy") || (submitButton && submitButton.disabled)) return;
            if (form.dataset.apiForm === "demo") playDemoActionRipple(form);
            if (reportFirstInvalidField(form)) submitForm(form);
            else if (form.dataset.apiForm === "demo") finishDemoAction(form, "error", "Invalid email address");
        }
        if (submitButton) submitButton.addEventListener("click", attemptSubmit);
    ...
    document.addEventListener("smtprs:themechange", rerenderTurnstileForTheme);
})();
