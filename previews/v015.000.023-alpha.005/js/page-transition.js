...
(function () {
    ...
    function writeArrivalState(url) {
        // Private assessment URLs already opt into arrival transitions. Do not
        // copy their bearer code into the cross-page session-storage handoff.
        if (/^\/careers\/assessments\/start\//.test(url.pathname)) {
            storageRemove(stateKey);
            return;
        }
    ...
    function setBusy(active) {
        ...
        if (busyCount === 0) hideLoader(false);
    }
    ...
    function isEligibleUrl(url) {
        ...
        var finalSegment = pathname.split("/").pop();

        if (url.origin !== window.location.origin) return false;
        if (pathname === window.location.pathname && url.search === window.location.search) return false;

        /* Support static .html pages plus clean directory and extensionless routes. */
        if (pathname === "/" || pathname.endsWith("/") || /\.html$/i.test(pathname)) return true;

        /* A final path segment with a file extension is an asset, not a page route. */
    ...
    function restoreArrivalTheme(state) {
        var fixedTheme = document.documentElement.getAttribute("data-route-theme");
        if (isValidTheme(fixedTheme)) {
    ...
    function initArrival() {
        ...
        var platformPage = isPlatformPath(window.location.pathname);
        var platformVisited = platformPage && platformHasBeenVisited();
        var usePlatformStartupLoader = !!state && platformPage && !platformVisited;
        var directArrival = root.getAttribute("data-route-arrival") === "true";

        if (platformVisited) {
            root.classList.add("site-platform-loader-skipped");
        }

        if (!state && !directArrival) {
            root.classList.add("site-is-ready");

            /* Returning homepage visits use the lightweight loader only when loading is slow. */
            if (platformVisited) {
                scheduleLoader();
        ...
        if (usePlatformStartupLoader) {
            ...
            /* Avoid flashing the route loader when the destination loads quickly. */
            scheduleLoader();
        }
