...
(function () {
    ...
    var CAMPAIGN = {
        ...
        dismissalSuppressionDays: 3,
        ctaSuppressionDays: 3,
        repeatUntilAuthenticated: true,
        idleRetryDelay: 1000,
        closeTransitionDuration: 220,
        darkLogoSource: "../../../shared/76/76010f0394048a52ec556035630d91c9204895f9c94f9fe83fb61d8fb9ecd180.svg",
        lightLogoSource: "../../../shared/53/5379caf98dc03f58295f0478bbea0ec723e97bd443ddd5f4bfc6e946ae659fbe.svg",
    ...
    function sessionCheckIsComplete() {
        return root.getAttribute("data-site-session-state") === "guest";
    }
    ...
    function ensurePromotionMarkup() {
        ...
        logoSource = root.getAttribute("data-theme") === "light"
            ? CAMPAIGN.lightLogoSource
            : CAMPAIGN.darkLogoSource;
                    ...
                    '<div class="campaign-promo-grid">',
                        '<div class="campaign-promo-copy">',
                            '<div class="campaign-promo-brand">',
                                '<img src="' + logoSource + '" data-theme-src-dark="' + CAMPAIGN.darkLogoSource + '" data-theme-src-light="' + CAMPAIGN.lightLogoSource + '" alt="" width="34" height="34" decoding="async">',
    ...
    function campaignIsSuppressed() {
        ...
        if (state.action === "converted") return true;
        if (state.action === "cta" && !CAMPAIGN.repeatUntilAuthenticated) return true;
        if (state.action !== "dismissed" && state.action !== "cta") return false;

        suppressionDays = state.action === "cta"
            ? CAMPAIGN.ctaSuppressionDays
            : CAMPAIGN.dismissalSuppressionDays;
        suppressionWindow = Math.max(0, suppressionDays) * MILLISECONDS_PER_DAY;
        return Date.now() - Number(state.timestamp) < suppressionWindow;
    }

    function recordAuthenticatedConversion() {
        var state;

        if (!userIsAuthenticated()) return false;

        state = readCampaignState();
        if (!state || state.campaignKey !== CAMPAIGN.campaignKey || state.action !== "converted") {
            writeCampaignState("converted", "authenticated-session");
        }
    ...
    /*
     * The console is dimensional in CSS at rest. On fine-pointer desktops this
     * adds a restrained mouse-follow tilt and moving highlight, throttled to one
     * paint per frame. Reduced-motion and touch inputs keep the static treatment.
     */
    function initializePromotionDepth() {
        var visual = backdrop.querySelector(".campaign-promo-visual");
        var panel = backdrop.querySelector(".campaign-promo-console");
        ...
        if (!visual || !panel || visual.getAttribute("data-depth-ready") === "true") return;

        visual.setAttribute("data-depth-ready", "true");
        ...
        function resetDepth() {
            ...
            visual.classList.remove("is-depth-active");
            panel.style.setProperty("--promo-tilt-x", "0deg");
            panel.style.setProperty("--promo-tilt-y", "0deg");
            panel.style.setProperty("--promo-light-x", "68%");
            panel.style.setProperty("--promo-light-y", "18%");
        }
        ...
        function paintDepth() {
            ...
            normalizedX = Math.max(-1, Math.min(1, ((pointerX - rect.left) / rect.width) * 2 - 1));
            normalizedY = Math.max(-1, Math.min(1, ((pointerY - rect.top) / rect.height) * 2 - 1));
            // tiltX = normalizedY * -1.65;
            // tiltY = normalizedX * 2.15;
            tiltX = normalizedY * -3.3;
            tiltY = normalizedX * 4.3;
            lightX = 50 + normalizedX * 18;
            lightY = 22 + normalizedY * 12;

            panel.style.setProperty("--promo-tilt-x", tiltX.toFixed(2) + "deg");
            panel.style.setProperty("--promo-tilt-y", tiltY.toFixed(2) + "deg");
            panel.style.setProperty("--promo-light-x", lightX.toFixed(1) + "%");
            panel.style.setProperty("--promo-light-y", lightY.toFixed(1) + "%");
        }

        function handlePointerMove(event) {
            pointerX = event.clientX;
            pointerY = event.clientY;
            visual.classList.add("is-depth-active");

            if (!animationFrame) {
                animationFrame = window.requestAnimationFrame(paintDepth);
            }
        }
        ...
        function bindDepth() {
            if (isBound || !hoverQuery.matches || motionQuery.matches) return;
            ...
            visual.addEventListener("pointerleave", resetDepth, { passive: true });
            isBound = true;
        }

        function unbindDepth() {
            if (!isBound) {
                resetDepth();
                return;
            }
            ...
            visual.removeEventListener("pointerleave", resetDepth);
            isBound = false;
            resetDepth();
        }

        function syncDepthBehavior() {
        ...
        if (typeof hoverQuery.addEventListener === "function") {
            hoverQuery.addEventListener("change", syncDepthBehavior);
            motionQuery.addEventListener("change", syncDepthBehavior);
        } else if (typeof hoverQuery.addListener === "function") {
            hoverQuery.addListener(syncDepthBehavior);
            motionQuery.addListener(syncDepthBehavior);
        }
    ...
    function tryToOpen() {
        if (recordAuthenticatedConversion()) {
            stopEligibilityTracking();
            return;
        }
    ...
    function handleMobileNavigationState(event) {
        if (event.detail && event.detail.open) return;

        window.clearTimeout(idleRetryTimer);
        idleRetryTimer = window.setTimeout(tryToOpen, 60);
    }
    ...
    function initializeCampaign() {
        ...
        window.addEventListener("paravane:sessionstatechange", handleSessionStateChange);

        if (recordAuthenticatedConversion() || !routeIsEligible() || campaignIsSuppressed()) return;
        ...
        window.addEventListener("paravane:mobile-nav-statechange", handleMobileNavigationState);
