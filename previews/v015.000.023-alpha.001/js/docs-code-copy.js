(function () {
    ...
    function bindCopyButton(button, code) {
    ...
    function addCopyControl(card, index) {
        ...
        bindCopyButton(button, code);
    }

    function addCommandCopyControl(command, index) {
        var code = command.querySelector("code");
        var existingButton = command.querySelector("[data-docs-copy-code]");
        var button;

        if (!code) return;

        if (!code.id) code.id = "dev-command-code-" + index;
        command.classList.add("has-copy-control");

        if (existingButton) {
            existingButton.setAttribute("aria-describedby", code.id);
            setButtonState(existingButton, "ready");
            bindCopyButton(existingButton, code);
            return;
        }

        button = document.createElement("button");
        button.className = "docs-copy-button dev-copy-button";
        button.type = "button";
        button.setAttribute("data-docs-copy-code", "");
        button.setAttribute("aria-describedby", code.id);
        button.innerHTML = '<i class="fa-regular fa-copy" aria-hidden="true"></i><span>Copy</span>';
        setButtonState(button, "ready");
    ...
    function init() {
        ...
        Array.prototype.slice.call(document.querySelectorAll(".page-developers .dev-command, .page-open-source .dev-command, .page-smtprs .dev-command")).forEach(addCommandCopyControl);
    }
