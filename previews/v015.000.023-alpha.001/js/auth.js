(function () {
    ...
    function completeVerificationPage(form, result) {
        ...
        if (actions) {
            ...
            if (nextLink) {
                if (active) {
                    nextLink.href = "../pages/auth/signin?next=%2Fpages%2Fapp%2Fapi-keys";
                    nextLink.textContent = "Sign in to create API key";
                } else if (billingRequired) {
                    nextLink.href = "../pages/auth/signin?next=%2Fpages%2Fapp%2Fbilling";
    ...
    function nextUrl(form) {
        ...
        var next = params.get("next") || form.dataset.successRedirect || "../pages/app/";
        return next.charAt(0) === "/" && next.charAt(1) !== "/" ? next : "../pages/app/";
    }
    ...
    async function submitForm(form) {
        ...
        try {
            ...
            if (kind === "forgot") {
                ...
                navigateWithTransition(successRedirect(form, "../pages/auth/password-reset-success"), 350);
                return;
            } else if (kind === "verify") {
