(function () {
    ...
    var endpoints = {
        login: "/z...PL...ZZ47E2A1...9C63...4F0D...8E5A...71C29B6F3D84/previews/v008.003.000/v1/auth/login",
        forgot: "/z...PL...ZZ47E2A1...9C63...4F0D...8E5A...71C29B6F3D84/previews/v008.003.000/v1/auth/forgot-password",
        reset: "/z...PL...ZZ47E2A1...9C63...4F0D...8E5A...71C29B6F3D84/previews/v008.003.000/v1/auth/reset-password",
        verify: "/z...PL...ZZ47E2A1...9C63...4F0D...8E5A...71C29B6F3D84/previews/v008.003.000/v1/auth/verify-email",
        resend: "/z...PL...ZZ47E2A1...9C63...4F0D...8E5A...71C29B6F3D84/previews/v008.003.000/v1/auth/resend-verification"
    };
    ...
    function completeVerificationPage(form, result) {
        ...
        var billingRequired = status === "active_billing_required";
        if (title) title.textContent = "Email verified.";
        if (copy) {
            copy.textContent = active
                ? "Your email is confirmed and Free API access is active. Check your inbox for the API key."
                : billingRequired
                    ? "Your email is confirmed. Sign in and complete billing before paid API access or API key creation is enabled."
                    : "Your email is confirmed. Your request is under review before API access is activated.";
        }
        if (button) {
    ...
    function nextUrl(form) {
        ...
        var next = params.get("next") || form.dataset.successRedirect || "/z...PL...ZZ47E2A1...9C63...4F0D...8E5A...71C29B6F3D84/previews/v008.003.000/pages/app/index.html";
        return next.charAt(0) === "/" && next.charAt(1) !== "/" ? next : "/z...PL...ZZ47E2A1...9C63...4F0D...8E5A...71C29B6F3D84/previews/v008.003.000/pages/app/index.html";
    }
    ...
    async function submitForm(form) {
        ...
        try {
            ...
            if (kind === "forgot") {
                ...
                navigateWithTransition(successRedirect(form, "/z...PL...ZZ47E2A1...9C63...4F0D...8E5A...71C29B6F3D84/previews/v008.003.000/pages/auth/password-reset-success.html"), 350);
                return;
            } else if (kind === "verify") {
                var verificationStatus = result && result.status;
                setStatus(
                    form,
                    verificationStatus === "active"
                        ? "Email verified. Free API access is active; check your email for the API key."
                        : verificationStatus === "active_billing_required"
                            ? "Email verified. Sign in and complete billing to activate paid API access."
                            : "Email verified. This request remains under review before API access is activated.",
                    "success"
                );
