/* ==========================================================================
   ...
   Used in:
     - index.html (applies to #large-header and #canvas-main)
   ...
   DEPENDENCIES:
     ...
     - Canvas element with id="canvas-main" inside #large-header
   --------------------------------------------------------------------------
   NOTES:
...
(function() {
    ...
    // Sets up canvas size and initial circle particles.
    // The hero height is controlled by CSS so the canvas, background mark,
    // and hero content all stay in the same layout system.
    function initHeader() {
        largeHeader = document.getElementById('large-header');
        canvas = document.getElementById('canvas-main');  // Main Page for index.html
    ...
    // Pause animation when the hero/header is no longer visible
    function scrollCheck() {
        var rect = largeHeader.getBoundingClientRect();
        animateHeader = rect.bottom >= 0 && rect.top <= window.innerHeight;
    }

    // Match the canvas to the rendered hero size without forcing hero height
    function resize() {
        width = largeHeader.clientWidth || window.innerWidth;
        height = largeHeader.clientHeight || window.innerHeight;
        target = {x: 0, y: height};
        canvas.width = width;
